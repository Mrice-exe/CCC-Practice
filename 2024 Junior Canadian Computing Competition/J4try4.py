...
if len(weirdinput) !=  int(weirdinputnumint):
    ...
    weirdinputnumwithout = []
    weirdinputnumwithout.extend(weirdinputnum)
    weirdinputnumwithout.remove(weirdinputnum[weirdinputnumint])
    if weirdinputnumwithout == weirdoutputnum:
        for p in range(weirdinputnumint):
            quite.remove(thecharacter)
        silly.extend(weirdinput)
        silly.remove(quite[0])
    elif weirdinputnumwithout == weirdoutputnum == numc:
        for p in range(weirdinputnumint):
            quite.remove(thecharacter)
        silly.extend(weirdinput)
        silly.remove(quite[0])
